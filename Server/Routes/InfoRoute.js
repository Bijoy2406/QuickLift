    ...
    try {
        const sql = `
            SELECT rr.id AS request_id, u.name AS user_name, u.email AS user_email, 
                   rr.pickup_location, rr.dropoff_location, rr.status, rr.requested_at,
                   r.name AS rider_name, r.email AS rider_email, riders.car_number, 
                   pricing.final_fare, transaction.payment_status, transaction.transaction_id,
                   feedback.user_rating, feedback.rider_rating, feedback.comments
            FROM ride_requests rr
            INNER JOIN users u ON rr.user_id = u.id
            LEFT JOIN ride_assignments ra ON rr.id = ra.request_id
            LEFT JOIN riders ON ra.rider_id = riders.id
            LEFT JOIN users r ON riders.user_id = r.id
            LEFT JOIN pricing ON rr.id = pricing.request_id
            LEFT JOIN transaction ON rr.id = transaction.request_id
        ...
        db.query(sql, (error, results) => {
            if (error) return res.status(500).json({ success: false, message: "Server Error", error });
            return res.status(200).json({ success: true, rideHistory: results });
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: "Server Error", error });
    }
...
// Get canceled rides
router.get("/canceled-rides", async (req, res) => {
    try {
        const sql = `
            SELECT rr.id AS request_id, u.name AS user_name, u.email AS user_email, 
                   rr.pickup_location, rr.dropoff_location, rr.status, rr.requested_at,
                   r.name AS rider_name, r.email AS rider_email, riders.car_number,
                   cancellations.canceled_by, cancellations.reason
            FROM ride_requests rr
            INNER JOIN users u ON rr.user_id = u.id
            LEFT JOIN ride_assignments ra ON rr.id = ra.request_id
            LEFT JOIN riders ON ra.rider_id = riders.id
            LEFT JOIN users r ON riders.user_id = r.id
            LEFT JOIN cancellations ON rr.id = cancellations.request_id
            WHERE rr.status = 'Canceled'
        `;

        db.query(sql, (error, results) => {
            if (error) return res.status(500).json({ success: false, message: "Server Error", error });
            return res.status(200).json({ success: true, canceledRides: results });
        });
    } catch (error) {
        return res.status(500).json({ success: false, message: "Server Error", error });
    }
